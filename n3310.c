...
void LcdString_4(unsigned int func){  // печатаем четырехзначное число
byte k = 0;
...
for ( byte i = 0; i < 4; i++){
 ...
 else LcdChr (FONT_1X, 0x20);// печатаем пустое место если впереди ноль
 func=func-tetra*n;}
}

void LcdString_2(unsigned int func){  // печатаем двух значное число
byte k = 0;
if (func>99) func=99;
unsigned int n=100;
for ( byte i = 0; i < 2; i++){
...
void LcdString_no_nul_2(unsigned int func){  // печатаем двух значное число
byte k = 0;
if (func>99) func=99;
unsigned int n=100;
for ( byte i = 0; i < 2; i++){
