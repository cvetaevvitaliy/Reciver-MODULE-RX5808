...
void SPI_Init(void)
{
  ...
  /*разрешение spi, (1<<DORD)младший бит вперед,мастер, режим 0 (0<<CPHA)(защелка по переднему фронту)
  (1<<MSTR) - ведущый (master)*/
  SPCR |= (1<<SPE)|(1<<DORD)|(1<<MSTR)|(0<<CPOL)|(0<<CPHA)|(1<<SPR1)|(0<<SPR0);
  SPSR |= (0<<SPI2X);
}
